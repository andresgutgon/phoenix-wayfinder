defmodule Wayfinder.Typescript.Helpers do
  ...
  @type build_function_args :: %{
          ...
          required(:url_arguments) => BuildParams.argument(),
  ...
  def build_http_function(%{
    ...
    (#{function_args(args, optional_args)}options?: #{function_opts()}): {
      ...
      url: #{action}.url(#{url_args(url_arguments, args)}options),
      method: '#{method}',
    })
    """
  end
  ...
  def function_args(args, true) do
    "args?: #{args}, "
  end

  defp url_args(nil, _args), do: ""
  defp url_args(_url_args, nil), do: "undefined, "
  defp url_args(_url_args, _args), do: "args, "
